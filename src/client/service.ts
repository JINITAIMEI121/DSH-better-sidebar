...
/**
 * The builtin kinds whose seeds carry a real workspace path: their pages
 * resolve addresses against the session cwd (the editor renders the file,
 * the diff page its change). For every other kind `seed.path` is tab data —
 * #643: the 0.19 native surface used to send any path-bearing open through
 * `openResource`, which resolves the path against the session cwd and 400s
 * for custom third-party tabs whose `type` has no file semantics.
 */
const FILE_TAB_KINDS = new Set(['editor', 'diff'])
...
export function createBetterSidebarService(store: SidebarStore): BetterSidebarService {
  ...
  const openTab = (seed: OpenTabSeed, scope?: SessionScope): void => {
    ...
    if (surface !== undefined && seed.target !== 'bottom') {
      ...
      if (seed.path !== undefined && FILE_TAB_KINDS.has(seed.type)) {
        surface.openResource({
          sessionId: targetSessionId,
          address: surface.fileAddress(targetSessionId, scope?.cwd, seed.path),
          revealIfOpened: true,
        })
      } else if (seed.path === undefined && seed.type === 'editor') {
